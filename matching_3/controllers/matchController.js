...
function firstFilter(idUser, users, usersAns){
    ...
    if(locationPref != 'any'){
        console.log(user.preferStay);
        const matchLoc = findMatchList(idUser, "preferStay", user.preferStay, users);
        filterOne = filterOne.filter(filterOne => -1 !== matchLoc.indexOf(filterOne));
